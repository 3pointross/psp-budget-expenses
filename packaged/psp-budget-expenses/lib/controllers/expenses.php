...
add_action( 'init', 'psp_custom_set_be_permissions' );
function psp_custom_set_be_permissions() {

     if( !current_user_can('manage_options') || !isset($_GET['psp_get_be_permissions']) ) {
          return;
     }

     $role_slug = $_GET['psp_get_be_permissions'];
...
function psp_budget_set_permissions() {
    ...
    if( $psp_budget_perms && !isset($_GET['reset_psp_be_permissions']) ) {
        return;
    }
